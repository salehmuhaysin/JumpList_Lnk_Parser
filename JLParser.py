	...
	def parse_DestList(self, data):
		...
		for entry in range(0 , DestList_header['Total_Current_Entries']):
			...
			DestList_Entry = {
				...
				'NetBIOS' 					: self.unpack_int_l(entry_data[72:88] , 'ascii'),
	...
	def parse_Lnk(self, stream):
		...
		if "HasLinkInfo" in LnkStreamHeader['Data_Flags']:
			...
			if self.unpack_int_l(volume_info[12:16]) <= 16:
				...
				Location_Info_Details['Volume_Label'] = self.unpack_int_l(volume_label , 'ascii')
			else:
			...
			Location_Info_Details['Local_Path'] = self.unpack_int_l(stream[local_path_offset:local_path_offset+stream[local_path_offset:].find(b'\0')], 'ascii')
			...
			if self.unpack_int_l(network_share_stream[8:12]) > 20:
				...
				Location_Info_Details['Network_Share_Name'] 	= self.unpack_int_l(stream[Network_Share_Name_offset:Network_Share_Name_offset+stream[Network_Share_Name_offset:].find(b'\0')] , 'ascii')
			...
			if 'ValidDevice' in Location_Info_Details['Network_Share_Flags'] and self.unpack_int_l(network_share_stream[12:16]) != 0:
				...
				Location_Info_Details['Network_Device_Name'] = self.unpack_int_l(stream[Network_Device_Name_offset:Network_Device_Name_offset+stream[Network_Device_Name_offset:].find(b'\0')] , 'ascii')
			...
			Location_Info_Details['Common_Path']= self.unpack_int_l(stream[common_path_offset:common_path_offset+stream[common_path_offset:].find(b'\0')] , 'ascii')
